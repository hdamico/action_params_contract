...
RSpec.describe ActionParamsContract::ContractGenerator do
  ...
  describe ".call" do
    context "when called with a fresh schema name" do
    ...
    context "when called twice with the same schema name" do
      it "reuses the existing module" do
        described_class.call("TestContractSchema") do
          params do
            required(:name).filled(:string)
          end
  ...
  describe ".call concurrent registration" do
    let(:thread_count) { 10 }
  ...
  describe ".build_contract" do
    ...
    context "when the built contract validates valid params" do
    ...
    context "when the built contract validates invalid params" do
